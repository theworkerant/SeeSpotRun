SeeSpotRun.Session = DS.Model.extend SeeSpotRun.SessionAssociationsMixin,
  ...
  decode_skills: ->
    ...
    for i in [0..split.length-1]
      if i is 0
        split[i] = Mask.parse(split[i], "skill")
      else
        split[i] = Mask.parse(split[i], "condition")
      
    @set("skill_conditions", split.slice(1))
    @set("skills_mask", split[0])
  
  skills_mask_obs: Em.observer ->
    encoded = binaryToHex(@get("skills_mask"))
    @get("skills").forEach (skill) -> encoded += ":#{binaryToHex(skill.get("conditions_mask"))}" if skill.get("conditions_mask")
